...
# ── Auth check ────────────────────────────────────────────────────────────────
echo -e "${BOLD}${CYAN}┌─────────────────────────────────────────────────┐${NC}"
echo -e "${BOLD}${CYAN}│   AWS Infrastructure Inventory Tool  (v1_5-5)   │${NC}"
echo -e "${BOLD}${CYAN}└─────────────────────────────────────────────────┘${NC}"
echo ""
...
# Usage:
#   terraform init
#   terraform plan -out=generated.file
#   terraform apply "generated.file"
...
while IFS= read -r zone; do
  ...
  zone_full_id=$(echo "$zone" | jq -r '.Id // ""')                         # /hostedzone/XXXXX — required by Terraform import
  id=$(echo "$zone_full_id" | sed 's|/hostedzone/||'); [[ -z "$id" ]] && continue  # bare ID for API calls
  ...
  # For private zones fetch the actual VPC associations and write real vpc {} blocks
  # Without these blocks Terraform will try to disassociate every VPC on the next plan
  if [[ "$private" == "true" ]]; then
    while IFS= read -r assoc; do
      [[ -z "$assoc" || "$assoc" == "null" ]] && continue
      assoc_vpc_id=$(echo "$assoc"     | jq -r '.VPCId // ""')
      assoc_vpc_region=$(echo "$assoc" | jq -r '.VPCRegion // ""')
      [[ -z "$assoc_vpc_id" ]] && continue
      attrs="${attrs}  vpc {\n    vpc_id     = \"${assoc_vpc_id}\"\n    vpc_region = \"${assoc_vpc_region}\"\n  }\n"
    done < <(aws_safe route53 get-hosted-zone --id "$id"       | jq -c '.VPCs[]?' 2>/dev/null || true)
  fi

  [[ -n "$tags_hcl" ]] && attrs="${attrs}${tags_hcl}"

  # Use full /hostedzone/ID as import ID — bare zone name causes Terraform to create a duplicate
  # Include zone ID in label to disambiguate public/private pairs with the same name
  emit_resource "route53" "aws_route53_zone" "$zone_full_id" \
    "$(sanitise_label "zone_${name}_${id}")" "global" "$(printf '%b' "$attrs")"
  ((COUNT++)) || true
