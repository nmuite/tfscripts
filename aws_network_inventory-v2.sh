...
# USAGE:
#   chmod +x aws_network_inventory.sh
#   ./aws_network_inventory.sh                          # default profile + configured region
#   ./aws_network_inventory.sh --profile myprofile      # named profile
#   ./aws_network_inventory.sh --region eu-west-1       # explicit region (repeatable)
#   ./aws_network_inventory.sh --all-regions            # scan every enabled region
#   ./aws_network_inventory.sh --concurrency 4          # parallel region workers (default 4)
#   ./aws_network_inventory.sh --no-import-blocks       # skip imports.tf generation
#   ./aws_network_inventory.sh --no-stubs               # import blocks only — no resource blocks
#                                                       # allows: terraform plan -generate-config-out=generated.tf
#
# WORKFLOW — two modes:
#   Mode A (default, with stubs):
#     terraform init && terraform apply
#
#   Mode B (--no-stubs, let Terraform generate config):
#     terraform init
...
ALL_REGIONS=false
IMPORT_BLOCKS=true
WRITE_STUBS=true       # set false with --no-stubs to skip resource blocks and let Terraform generate them via -generate-config-out
CONCURRENCY=4
...
while [[ $# -gt 0 ]]; do
  case $1 in
    ...
    --no-stubs)         WRITE_STUBS=false;     shift   ;;
...
write_tf_stub() {
  ...
  $WRITE_STUBS || return 0   # skip resource block generation when --no-stubs passed
...
if $WRITE_STUBS; then
  echo -e "Resource stubs: ${GREEN}enabled${NC} (use terraform apply directly)"
else
  echo -e "Resource stubs: ${YELLOW}disabled${NC} (run: terraform plan -generate-config-out=generated.tf)"
fi
echo ""
