...
for SUB_ID in "${SUBSCRIPTION_IDS[@]}"; do
  ...
  while IFS= read -r sp; do
    ...
    attrs="  display_name  = \"${name}\"\n  # client_id   = \"${app_id}\"  # set if managing the app registration too"
  ...
  for RG in "${RESOURCE_GROUPS[@]}"; do
    ...
    while IFS= read -r vm; do
      ...
      if [[ "$os" == "Linux" && "$disable_pw" == "true" ]]; then
        ...
        attrs="${attrs}\n  # admin_password = \"<set via tfvars or Vault — never hardcode>\""
      fi
    ...
    while IFS= read -r agw; do
      ...
      attrs="${attrs}\n\n  # TODO: configure gateway_ip_configuration, frontend_ip_configuration,\n  #        frontend_port, backend_address_pool, backend_http_settings,\n  #        http_listener, request_routing_rule after import"
      [[ -n "$tags_hcl" ]] && attrs="${attrs}\n${tags_hcl}"
    ...
    while IFS= read -r srv; do
      ...
      attrs="  name                          = \"${name}\"\n  resource_group_name           = \"${RG}\"\n  location                      = \"${loc}\"\n  version                       = \"${version}\"\n  administrator_login           = \"${admin}\"\n  # administrator_login_password = \"<set via tfvars — never hardcode>\"\n  minimum_tls_version           = \"${min_tls}\"\n  public_network_access_enabled = $([ "$public_net" == "Enabled" ] && echo true || echo false)"
      if [[ -n "$identity_type" && "$identity_type" != "null" ]]; then
        attrs="${attrs}\n\n  identity {\n    type = \"${identity_type}\"\n  }"
      fi
      [[ -n "$tags_hcl" ]] && attrs="${attrs}\n${tags_hcl}"
    ...
    while IFS= read -r cosmos; do
      ...
      attrs="${attrs}\n\n  # TODO: add geo_location blocks for each region: ${geo_locs}"
      [[ -n "$tags_hcl" ]] && attrs="${attrs}\n${tags_hcl}"
